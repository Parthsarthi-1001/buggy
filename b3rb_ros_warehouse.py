...
#due to this counter value will update in the next itteration to find index of next shelf
            self.qr_angle=float(self.qr_random.split("_")[1])
    ...
    def explore(self, img,map_array, map_info):
        ...
        if len(frontiers)>5:
            ...
            while 1:
                ...
                self.node_y+= np.sin(np.deg2rad(self.qr_angle))*dist*self.self.dirn
                ...
                if (0<self.node_x<width and 0<self.node_y<height):
                    ...
                    if img[self.node_y][self.node_x]==-1  : #unexplored
                        ...
                        self.logger.info("going in the self.dirn of next shelf on explored paimg")
                    ...
                    if dist<3 :
                        ...
                        self.logger.info("no unexplored point found in the self.dirn of next shelf,changing self.dirn ")
        ...
        for i, cnt in enumerate(ct):
            ...
            if   0.5 < dim[0]/dim[1] < 0.8 and area / (dim[0]*dim[1]) > 0.8:#
                ...
                if M["m00"] != 0:
                    ...
                    o1,o2= self.shelf_coords(img,cx, cy, angle, dist, m)
    ...
    def find_shelves(self, img,th, height, width, map_info):
        ...
        # self.logger.info(f"Nothing found in frontiers; count = {self.full_map_explored_count}")
        # if self.flag == 0:
    ...
    def global_map_callback(self, message):
        ...
        self.logger.info(f"last shelf coordinates {self.current_com_x},{self.current_com_y}")
